...
module.exports = async ({ github, context, core }) => {
  ...
  function makeBadge(record) {
    ...
    return [
      ...
      `  <title>${safeId} · @${safeHolder} · ${safeRight}</title>`,
  ...
  for (const [userId, prs] of groups) {
    ...
    for (const item of relatedExisting) {
      ...
      for (const code of old.achievements || []) achievements.add(code);
      for (const flag of old.prestige || []) prestige.add(flag);
      tuitionCreative ||= Boolean(old.tuition?.creative);
      tuitionDress ||= Boolean(old.tuition?.dress);
    ...
    for (const pr of prs) {
      ...
      if (/<!--\s*tuition:\s*dress\s*-->/i.test(body)) {
        tuitionDress = true;
        achievements.add('TUITION_PAID_DRESS');
      }
      if (/<!--\s*tuition:\s*creative\s*-->/i.test(body)) {
    ...
    const record = {
      ...
      tuition: { creative: tuitionCreative, dress: tuitionDress },
    ...
    for (const pr of prs) {
      try {
        ...
        if (record.aethra_fanwork_pass?.active) lines.push('', '✨ **AETHRA_FANWORK_PASS · ACTIVE**');
  ...
  if (JSON.stringify(parsedRoster) !== JSON.stringify(roster)) {
    await putText(
      rosterPath,
      `${JSON.stringify(roster, null, 2)}\n`,
      'chore(credentials): refresh Mission Control roster',
      existingRoster?.sha || null
    );
  }

  core.notice(`Mission Control reconciliation complete: ${canonicalRecords.length} active credential(s).`);
};
